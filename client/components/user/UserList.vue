  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        isShowList: state => state.user.showList,
        loading: state => state.config.loading,
      }),
      ...mapGetters({
        items: "user/items",
        item: "user/item",
        hasItemForm: "user/hasItemForm",
        dataItem: "user/itemForm",
        itemSelected: "user/itemSelected",
      }),
      headers () {
        return [
          { text: '#', value: 'id' },
    ...
    methods: {
      ...
      close () {
        ...
        this.$store.dispatch('user/showList',false);
        
      },
      dialogItem (item, mode) {  
        
        this.$store.dispatch('user/'+mode);

        this.$store.dispatch('user/item',Object.assign({}, item));  
        this.$store.dispatch('user/indexItem',this.items.indexOf(item));
      },
