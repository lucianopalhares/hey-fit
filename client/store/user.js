...
export const actions = {
  ...
  async fetchItems({ commit }, payload = {}) {
    ...
    const items = await this.$axios.$post('/api/filtra-usuarios',{where:payload}).then(response => {
  ...
  async store({ commit }, item) {
    ...
    await this.$axios.$post('/api/usuario',item).then(response => {
  ...
  async update({ commit }, item) {
    ...
    if(Object.keys(item).length > 0 && item.id != undefined) {  
      ...
      await this.$axios.$put('/api/usuario/'+item.id,item).then(response => {
  ...
  async deleteItem({ commit }, item) {
    ...
    if(Object.keys(item).length > 0 && item.id != undefined) { 
      ...
      await this.$axios.$delete('/api/usuario/'+item.id).then(response => {
